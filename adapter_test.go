...
func testGetPolicy(t *testing.T, e *casbin.Enforcer, res [][]string) {
	t.Helper()
	myRes := e.GetPolicy()
	for i := len(myRes)/2 - 1; i >= 0; i-- {
		opp := len(myRes) - 1 - i
		myRes[i], myRes[opp] = myRes[opp], myRes[i]
	}
...
func TestAdapter(t *testing.T) {
	...
	a, err := NewAdapter()
	if err != nil {
		t.Fatalf("Expected NewAdapter() to be successful; got %v", err)
	}
...
func TestAdapterWithOptions(t *testing.T) {
	...
	// Create an adapter and an enforcer.
	// NewEnforcer() will load the policy automatically.
	a, err := NewAdapter(Collection("mycasbincollection"))
	if err != nil {
		t.Fatalf("Expected NewAdapter() to be successful; got %v", err)
	}
	e, err := casbin.NewEnforcer("examples/rbac_model.conf", a)
